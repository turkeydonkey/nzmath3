...
from sandbox.algebraiccurve._sup_elliptic import *
import sandbox.group as group
import sandbox.grouplikeset as grouplikeset

_log = logging.getLogger('sandbox.algebraicCurve.elliptic')

# old interface inclusion ( ULTIM.GOAL::whole rewrite )
import nzmath.elliptic as elliptic

ECoverQ = elliptic.ECoverQ
...
class ECGeneric:
    ...
    def __str__(self):
        ...
        if self.a6:
            dictR[0] = self.a6
        return ("%s = %s" %
                (str(strMultiPoly(dictL)), 
                 str(strUniPoly(UniVarPolynomial(dictR, self.basefield)))))
    ...
    def changeCurve(self, V):
        ...
        if self.ch == 0:
            ...
            for v in V:
                if not isinstance(v, int) or not (v in self.basefield):
    ...
    def divPoly(self, Number=None):
        ...
        if not Number:
            ...
            E = self.simple()
            e = 4 * E.cubic
            f[3] = UniVarPolynomial({4:3*one,
                                     2:6*E.a,
                                     1:12*E.b,
                                     0:-E.a**2},
                                    self.basefield)
            f[4] = UniVarPolynomial({6:2*one,
                                     4:10*E.a,
                                     3:40*E.b,
                                     2:-10*E.a**2,
                                     1:-8*E.a*E.b,
...
class ECoverGF(ECGeneric):
    ...
    def _sub1(self, k, t, lth_div):
        ...
        D = self.division_polynomials
        f = {}
        f[0] = PolyMulRed([D[k-1], D[k+1]], lth_div)
        f[3] = PolyMulRed([D[k], D[k]], lth_div)
        f[4] = PolyMulRed([t, f[3]], lth_div)
        if k % 2 == 0:
            P = PolyMulRed([f[4], self.cubic], lth_div) + f[0]
        else:
            P = f[4] + PolyMulRed([f[0], self.cubic], lth_div)
        return f, P
    ...
    def _sub2(self, k, g, t, lth_div):
        ...
        """
        D = self.division_polynomials
        fk = PolyMulRed([4 * g, t, D[k]], lth_div)
        f1 = PolyMulRed([D[k-1], D[k-1], D[k+2]], lth_div)
        f2 = PolyMulRed([D[k-2], D[k+1], D[k+1]], lth_div)
        if k % 2 == 0:
            cubic = self.cubic
            P = PolyMulRed([fk, cubic, cubic], lth_div) - f1 + f2
        else:
            P = fk - f1 + f2
        return P

    def _Z_x(self, t, D, e_q, bfsize, lth_div):
        d = PolyPow(D[t], 2 * bfsize, lth_div)
        n = PolyPow(PolyMulRed([D[t-1], D[t+1]], lth_div), bfsize, lth_div)
        if t % 2 == 0:
            d = PolyMulRed([e_q, d], lth_div)
        else:
            n = PolyMulRed([e_q, n], lth_div)
        return d, n

    def _Z_y(self, t, D, g0, bfsize, lth_div):
        E = self.cubic
        if t % 2 == 0:
            d = PolyPow(PolyMulRed([4 * E, E, D[t], D[t], D[t]], lth_div), bfsize, lth_div)
        else:
            d = PolyPow(PolyMulRed([4 * D[t], D[t], D[t]], lth_div), bfsize, lth_div)
        z0 = PolyPow(PolyMulRed([D[t-1], D[t-1], D[t+2]], lth_div) - PolyMulRed([D[t-2], D[t+1], D[t+1]], lth_div), bfsize, lth_div)
        n = PolyMulRed([g0, z0], lth_div)
        return d, n
...
##     def allPoint(self, O):
##         p = self.ch
##         x = polynomial.OneVariableMonomial("x", coeffring=self.basefield)
##         sform = self.simple()
...
class ECTorsionPointGroup(group.Module):
    """
    Torsion Point Group of Elliptic curve over Galois Field.
    """
    def __init__(self, curve):
        group.AbelianGroup.__init__(self)
        if not isinstance(curve, ECGeneric):
            raise ValueError("curve must be ECGeneric instances")
        self.curve = curve
        self._zero = None

    def __contains__(self, element):
        try:
            return self.curve.WhetherOn(element.coordinate)
        except:
            return False

    def __str__(self):
        return "%s over %s" % (self.__class__.__name__, repr(self.curve))

    def __repr__(self):
        return "%s(%s)" % (self.__class__.__name__, repr(self.curve))

    def randomElement(self):
        return ECTorsionPoint(self.curve.point(), self.curve)

    def add(self, former, after, _oncheck=True):

        P = former.coordinate
        Q = after.coordinate
        zero = self.curve.basefield.zero
        if (P == [zero]) and (Q != [zero]):
            return ECTorsionPoint(Q, self.curve, _oncheck)
        elif (P != [zero]) and (Q == [zero]):
            return ECTorsionPoint(P, self.curve, _oncheck)
        elif (P == [zero]) and (Q == [zero]):
            return self.zero
        ...
        if self.ch == 0:
            ...
            y3 = -(s+self.a1)*x3-t-self.a3
            R = [x3, y3]
            return ECTorsionPoint(R, self.curve, _oncheck)
        else:
            if P[0] == Q[0]:
                if P[1]+Q[1]+self.a1*Q[0]+self.a3 == 0:
                    return self.zero
    ...
    def sub(self, former, after, _oncheck=True):
        P = former.coordinate
        Q = after.coordinate
        zero = self.curve.basefield.zero
        if (P != [zero]) and (Q == [zero]):
        ...
        x = Q[0]
        y = -Q[1]-self.a1*Q[0]-self.a3
        R = ECTorsionPoint([x, y], self.curve, _oncheck)
        if (P == [zero]) and (Q != [zero]):
            return R
        else:
            return self.add(former, R, _oncheck)

    def negative(self, element, _oncheck=True):
        return self.sub(self.zero, element, _oncheck)

    def scalarmul(self, element, scalar):
        """
        this returns [scalar]*element
        """
        if not element in self:
            raise TypeError("Point not on curve") 
        if scalar >= 0:
            l = arith1.expand(scalar, 2)
            Q = self.zero
            for j in range(len(l)-1, -1, -1):
                Q = self.add(Q, Q, False)
                if l[j] == 1:
                    Q = self.add(Q, element, False)
            return Q
        else:
            l = arith1.expand(-scalar, 2)
            Q = self.zero
            for j in range(len(l)-1, -1, -1):
                ...
                if l[j] == 1:
                    Q = self.add(Q, element, False)
            return self.sub(self.zero, Q, False)
    mul = scalarmul

    def elementOrder(self, element):
        return Group.elementOrder(self, element)

    @classmethod
    def getInstance(cls, value):
        return AlgebraicSystem.getInstance(cls, value)
    
    # properties
    def _getOrder(self):
        return Group._getOrder(self)

    def _getZero(self):
        if not self._zero:
            self._zero = ECTorsionPoint([self.curve.basefield.zero], self.curve, False)
        return self._zero
    zero = property(_getZero, None, None, "additive unit.")
    identity = zero


class ECTorsionPoint(group.ModuleElement):
    """
    Torsion Point on Elliptic curve over Galois Field.
    """
    def __init__(self, coordinate, curve, _oncheck=True):
        if not isinstance(curve, ECGeneric):
            raise ValueError("curve must be ECGeneric instances")
        if _oncheck and not curve.WhetherOn(coordinate):
            raise ValueError("coodinate must be on curve")
        self.curve = curve
        self.coordinate = coordinate

    def getModule(self):
        return self.curve.getInstance(self.curve.coefficient, self.curve.basefield)

    getGroup = getAdditiveGroup = getModule
